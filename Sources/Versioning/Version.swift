...
/// Version
///
/// A type for storing the app version in a structured object. 
/// Used to assemble the `UserAgent` HTTP header and for checking the app version.
public struct Version: CustomStringConvertible {
    let major: Int
    let minor: Int
    let increment: Int

    /// Failable initialiser for `Version` which accepts a single String argument. 
    /// Because it is failable, it returns an optional `Version`.
    ...
    public init(from decoder: Decoder) throws {
        ...
        guard let version = Version(string: string) else {
            throw DecodingError.dataCorrupted(
                .init(codingPath: container.codingPath, debugDescription: "Incorrect Version Format")
            )
        }
        self = version
    }
